...
1. **Data Loading:** Open `SpikeProteins.fa` (or `.fas`) in a sequence analysis software (e.g., MEGA).
2. **Alignment:** Perform MSA using ClustalW or Muscle algorithms. Export alignment visualization (result in `MSA_16variants.png`).
3. **Phylogeny Inference:** Construct a phylogenetic tree (e.g., using NJ or ML). Export tree data files (`SpikeProteins.tree`, `SpikeProteins.dnd`).
4. **Visualization:** Open tree data files in a tree viewer (e.g., FigTree) and generate the finalized tree image (`Phylogenetic_Tree.png`).

## 👨‍💻 Author

* **GitHub:** [@OHfromH2O](https://github.com/OHfromH2O)
